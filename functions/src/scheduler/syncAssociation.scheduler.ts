...
import {updateTeamsSwissunihockey, updateClubsSwissunihockey} from "./utils/update.swissunihockey";
...
export async function updatePersistenceJobClubs(context: EventContext) {
  try {
    /* await updateSwissunihockey();
    await updateSwissVolleyball();
    await updateSwissHandball();
    await updateSwissTurnverband(); */
