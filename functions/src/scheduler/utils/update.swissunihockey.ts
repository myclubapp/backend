...
export async function updateTeamsSwissunihockey(): Promise<any> {
  ...
  for (const clubData of clubListRef.docs) {
    ...
    for (const team of teamData) {
      ...
      // Lade Logo nur hoch, wenn kein Logo existiert oder wenn es nicht von Cloudinary stammt
      ...
      // Update TeamRef in Club Collection
      ...
      for (const teamMember of teamMemberRef.docs) {
        ...
        // Defensive check: Ensure gameCenterPlayers is an array before calling find()
        const gameCenterPlayers = Array.isArray(team?.gameCenterPlayers) ? team.gameCenterPlayers : [];
        const gameCenterPlayer = gameCenterPlayers.find((profile: any) => {
...
/**
 * Lädt ein Bild von einer URL herunter und speichert es auf Firebase Storage
 * @param imageUrl Die URL des Bildes, das heruntergeladen werden soll
 * @param storagePath Der Pfad, unter dem das Bild in Firebase Storage gespeichert werden soll
 * @returns Die öffentliche URL des hochgeladenen Bildes
 */
async function uploadImageToFirebaseStorage(imageUrl: string, storagePath: string): Promise<string> {
  try {
    ...
    // Bild als Buffer einlesen
    const imageBuffer = await response.buffer();

    // Referenz zum Firebase Storage Bucket erstellen
    const bucket = storage.bucket();
    const file = bucket.file(storagePath);

    // Metadaten für den Upload setzen
    const metadata = {
      contentType: 'image/png', // Standard ist PNG, kann bei Bedarf angepasst werden
    ...
    // Datei öffentlich zugänglich machen
    await file.makePublic();

    // Öffentliche URL zurückgeben
    return `https://storage.googleapis.com/${bucket.name}/${storagePath}`;
  } catch (error) {
    logger.error(`Fehler beim Hochladen des Bildes: ${error}`);
    throw error;
  }
}
